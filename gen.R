...
distributions = list(
  ...
  list(function(width) rbeta(n, shape1 = runif(1, 0, 1000)[[1]], shape2 = runif(1, 0, 1000)[[1]]), "beta"),
...
normals = list(
  ...
  list(function(width) rnorm(n, mean = runif(1, -width, width)[[1]], sd = runif(1, 0, 10)[[1]]), "normal2"),
  list(function(width) rnorm(n, mean = runif(1, -width, width)[[1]], sd = runif(1, 0, 1)[[1]]), "normal3")
)
...
for (i in 1:(as.integer(count/2))) {
  ...
  probe = round(distribution_pair[[1]](width), precision)
  while (-Inf %in% probe || Inf %in% probe || all_equal(probe)) {
    probe = round(distribution_pair[[1]](width), precision)
  }
  probe = append(probe, 0)
  probe = append(probe, distribution_pair[[2]])
